...
func (s *Service) IngestHeartbeat(ctx context.Context, nodeID uuid.UUID, req HeartbeatRequest) error {
	...
	node, err := s.q.GetFleetNode(ctx, nodeID)
	if err != nil {
		return mapErr(err)
	}
	prev := node.Status
	_, err = s.q.UpdateFleetNodeHeartbeat(ctx, db.UpdateFleetNodeHeartbeatParams{
		ID:           nodeID,
		Status:       StatusOnline,
...
func (s *Service) RecordEvent(ctx context.Context, nodeID uuid.UUID, ev IngestEvent) (bool, error) {
	...
	inserted, err := s.q.InsertFleetEvent(ctx, db.InsertFleetEventParams{
		EventID:      ev.EventID,
	...
	if isRecovery {
		if open, err := s.q.GetOpenIncidentByDedup(ctx, dedup); err == nil {
			_, _ = s.q.ResolveFleetIncident(ctx, db.ResolveFleetIncidentParams{
				ID:          open.ID,
	...
	if open, err := s.q.GetOpenIncidentByDedup(ctx, dedup); err == nil {
		_, _ = s.q.TouchFleetIncident(ctx, db.TouchFleetIncidentParams{
			ID:          open.ID,
			LastEventID: pgUUID(ev.EventID),
		})
		return true, nil
	...
	_, err = s.q.CreateFleetIncident(ctx, db.CreateFleetIncidentParams{
		NodeID:       pgUUID(nodeID),
		DedupKey:     dedup,
...
func (s *Service) ListEvents(ctx context.Context, limit, offset int32) ([]EventResponse, error) {
	...
	rows, err := s.q.ListFleetEvents(ctx, db.ListFleetEventsParams{Limit: limit, Offset: offset})
	if err != nil {
...
func (s *Service) ListIncidents(ctx context.Context) ([]IncidentResponse, error) {
	rows, err := s.q.ListOpenFleetIncidents(ctx)
	if err != nil {
		return nil, mapErr(err)
	}
...
func (s *Service) EnqueueLocalEvent(ctx context.Context, ev IngestEvent) error {
	...
	if settings.Mode == ModeManagedNode && settings.NotificationMode == NotifyMaster {
		...
		_, err := s.q.InsertFleetOutbox(ctx, db.InsertFleetOutboxParams{
			EventID: ev.EventID,
			Payload: payload,
		})
		return mapErr(err)
...
// OnLocalIncident implements notifications.FleetEventSink.
