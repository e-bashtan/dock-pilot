...
func (w *PollingWorker) pollNode(ctx context.Context, node db.ServersNode) {
	...
	defer cancel()
	req, err := http.NewRequestWithContext(pctx, http.MethodGet, stringsTrimRight(node.BaseUrl)+"/api/servers/node/status", nil)
	if err != nil {
		return
	}
	...
	_, _ = w.svc.q.InsertServersSnapshot(ctx, db.InsertServersSnapshotParams{
		NodeID:           node.ID,
		CollectedAt:      now,
	...
	prev := node.Status
	_, _ = w.svc.q.UpdateServersNodePoll(ctx, db.UpdateServersNodePollParams{
		ID:           node.ID,
		Status:       StatusOnline,
...
func (w *PollingWorker) recomputeStatuses(ctx context.Context) {
	nodes, err := w.svc.q.ListServersNodes(ctx)
	if err != nil {
		return
	}
	now := time.Now().UTC()
	for _, n := range nodes {
		...
		if st != n.Status {
			_ = w.svc.q.UpdateServersNodeStatus(ctx, db.UpdateServersNodeStatusParams{ID: n.ID, Status: st})
...
func (w *OutboxWorker) tick(ctx context.Context) {
	...
	for _, row := range rows {
		req, err := http.NewRequestWithContext(ctx, http.MethodPost, stringsTrimRight(settings.MasterUrl)+"/api/servers/ingest/events", bytes.NewReader(row.Payload))
		if err != nil {
			continue
		}
		req.Header.Set("Authorization", "Bearer "+token)
...
func (w *HeartbeatWorker) tick(ctx context.Context) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, stringsTrimRight(settings.MasterUrl)+"/api/servers/ingest/heartbeat", bytes.NewReader(body))
	if err != nil {
		return
	}
	req.Header.Set("Authorization", "Bearer "+token)
	...
	go func() {
		...
		for {
			...
			case <-ctx.Done():
				return
			case <-t.C:
				_ = w.svc.q.DeleteOldServersSnapshots(ctx, time.Now().UTC().Add(-30*24*time.Hour))
			}
		}
	}()
}
