...
func billingDTOFromManual(bill db.ServersNodeBilling) *BillingDTO {
...
func nodeMatchHaystack(row db.ServersNode, hostname, hostIP string) string {
...
func (s *Service) mergeNodeSnapshotPayload(ctx context.Context, nodeID uuid.UUID, fields map[string]any) []byte {
	...
	prevHostIP := ""
	if snap, err := s.q.GetLatestServersSnapshot(ctx, nodeID); err == nil {
...
func (s *Service) resolveNodeBilling(
	...
	bill, billErr := s.q.GetServersNodeBilling(ctx, row.ID)
