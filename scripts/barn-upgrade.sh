...
# Reuse legacy Postgres data volume if present (rebrand used to create a fresh empty barn_pg).
PG_VOL_OVERRIDE=""
resolve_legacy_pg_volume() {
  local v
  for v in \
...
if LEGACY_PG_VOL="$(resolve_legacy_pg_volume)"; then
  # Only pin external volume when compose expects barn_pg (barn compose files).
  if grep -qE '^[[:space:]]*barn_pg:' "$COMPOSE" 2>/dev/null; then
    PG_VOL_OVERRIDE="${ROOT}/docker-compose.pg-volume.override.yml"
    cat >"$PG_VOL_OVERRIDE" <<EOF
# Generated by barn-upgrade.sh — keep panel data on the pre-rebrand volume.
volumes:
  barn_pg:
    external: true
    name: ${LEGACY_PG_VOL}
EOF
    log "Reusing legacy Postgres volume: ${LEGACY_PG_VOL}"
  fi
fi

compose() {
  if [[ -n "$PG_VOL_OVERRIDE" && -f "$PG_VOL_OVERRIDE" ]]; then
    docker compose -f "$COMPOSE" -f "$PG_VOL_OVERRIDE" "$@"
  else
...
# Drop the empty barn-postgres if we're switching back to a legacy volume name.
if [[ -n "$PG_VOL_OVERRIDE" ]]; then
