...
export default function FleetEventsPage() {
  ...
  const load = useCallback(async () => {
    try {
      const rows = await api.listFleetEvents();
      setEvents(rows);
      setError(null);
    } catch (e) {
      setError(e instanceof ApiError ? e.message : t("fleet.eventsLoadFailed"));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1>{t("fleet.eventsTitle")}</h1>
          <p className="muted" style={{ margin: "0.35rem 0 0" }}>
            {t("fleet.eventsSubtitle")}
          </p>
        </div>
        ...
        <div className="card" style={{ padding: 0, overflow: "hidden" }}>
          <div className="table-wrap">
            <table className="table">
              <thead>
                <tr>
                  <th>{t("fleet.eventTime")}</th>
                  <th>{t("fleet.eventSeverity")}</th>
                  <th>{t("fleet.eventTitle")}</th>
                  <th className="col-hide-mobile">{t("fleet.eventNode")}</th>
                  <th className="col-hide-mobile">{t("fleet.eventType")}</th>
                </tr>
              </thead>
              <tbody>
                {events.map((ev) => (
